...
# Set up logging
log_file_path = os.path.join(SAVE_FILE_PATH, 'UG_HESLB_2015_1.log')
        ...
        for j in range(pdf_length):
            ...
            if j == 0:
                for i, text in enumerate(page_text):
                    ...
                    row = []
                    if i == 1:
                        row = ['S/No', 'Name of Applicant', 'Gender', 'District of Origin', 'College / University', 'Course', 'Duration']
                        write_row_to_csv(row, SAVE_FILE_PATH, 'data_1')
                    else:
                        if text == '':
                            continue
...
def transform_string(input_string):
    pattern = r'(\d+)\s+(.*?)\s+([FM])\s+(.*?)\s+(Diploma.*?)\s*(\d+.+)'
