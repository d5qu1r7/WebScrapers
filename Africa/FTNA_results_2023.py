...
baselink = 'https://maktaba.tetea.org/exam-results/FTNA2023/ftna.htm'
SAVE_FILE_PATH = '' # FIXME: Change to the perminant file path the .csv will be stored in

# Set up logging os.path.dirname(__file__)
log_file_path = os.path.join(SAVE_FILE_PATH, 'FTNA_results_2023.log')
            ...
            for row_letters in all_rows_letters:
                ...
                for column_letters in full_row_letters:
                    ...
                    if j == 0:
                        ...
                        # Find the table of schools
                        table_school = page.locator('.main')

                        # Find all the schools
                        schools = table_school.locator('div').all()
...
#----------------------------------------------------------- Loop Through Schools -----------------------------------------------------------#
                        ...
                        for school in schools:
                            ...
                            try:
                                name_school = school.locator('a').inner_text(timeout=5000)
...
#----------------------------------------------------------- Loop Through Tables In School Records -----------------------------------------------------------#
                            ...
                            for table_school_table in table_school_tables:
                                ...
                                if m == 1: # This skips the second table that is blank
                                    m += 1
                                    continue
